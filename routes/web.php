...
use App\Http\Controllers\Admin\{
    ...
    ReportController,
    SettingControlleruse ,IncomingGoodController,OutgoingGoodController,
};
...
Route::resource('incoming-goods', IncomingGoodController::class)->only([
   ...
   Route::prefix('admin')->name('admin.')->middleware(['auth'])->group(function () {
    ...
    // --- DEFINISI ROUTE BARANG KELUAR (YANG BENAR) ---

    // 1. Definisikan route print.
    // Karena ada di dalam grup name('admin.'), namanya otomatis menjadi 'admin.outgoing-goods.print'
    Route::get('outgoing-goods/{outgoingGood}/print', [OutgoingGoodController::class, 'print'])->name('outgoing-goods.print');
     Route::resource('incoming-goods', IncomingGoodController::class);
     Route::resource('outgoing-goods', OutgoingGoodController::class); // Hapus ->only([...]) agar semua method aktif
     Route::get('outgoing-goods/report/pdf', [OutgoingGoodController::class, 'generatePDF'])->name('outgoing-goods.report.pdf');
 Route::get('incoming-goods/report/pdf', [IncomingGoodController::class, 'generatePDF'])->name('incoming-goods.report.pdf');
    

    // 2. Definisikan route resource TANPA ['as' => 'admin']
    // Grup name('admin.') akan menanganinya secara otomatis.
    Route::resource('outgoing-goods', OutgoingGoodController::class)->only([
        'index', 'create', 'store', 'show'
    ]);
...
Route::resource('outgoing-goods', OutgoingGoodController::class, [
    'as' => 'admin' // Menambahkan prefix 'admin' ke nama route
])->only([
    'index', 'create', 'store', 'show'
]);
